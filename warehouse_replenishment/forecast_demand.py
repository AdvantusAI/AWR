...
def run_forecast(
    ...
    Args:
        ...
        periods: Number of periods to consider (defaults to value from settings.ini)
    ...
    # Get periods from settings if not provided
    if periods is None:
        from warehouse_replenishment.config import Config
        config = Config()
        periods = config.get_int('BUSINESS_RULES', 'periods', 12)
            ...
            for item in items:
                try:
                    ...
                    if update and not dry_run:
                        ...
                        # Save forecast history to item_forecast table
                        try:
                            # Get current period and year
                            periodicity = item.forecasting_periodicity or company_settings['forecasting_periodicity_default']
                            current_period, current_year = forecast_service.get_current_period(periodicity)
                            forecast_service.save_forecast_history(
                                ...
                                seasonality_applied=bool(seasonal_indices),
                                seasonal_profile_id=item.demand_profile if seasonal_indices else None,
                                notes=f"Generated by M8 predict.AI, using {periods} periods of history"
                            )
                        except Exception as e:
                            logger.warning(f"Could not save forecast history for item {item.item_id}: {str(e)}")
